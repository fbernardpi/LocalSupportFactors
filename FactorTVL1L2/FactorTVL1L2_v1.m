function [A,Z,B,obj,A_norm,Z_norm,B_norm,L_stats] = ...
	FactorTVL1L2_v1(X,Da,Db,param,num_iter)
%[A,Z,B,obj,A_norm,Z_norm,B_norm,L_stats] = ...
% FactorTVL1L2_v1(X,Da,Db,param,num_iter)
%
...
% modified by Florian Bernard (2015)
...
% general proximal operator for all variables ...
if ( ~isfield(param, 'proximalOperator') )
	proximalOperator = @proximalTVL1L2;
else
    proximalOperator = param.proximalOperator;
end

% ... which can be overwritten be these:
if ( ~isfield(param, 'proximalOperatorA') )
	proximalOperatorA = proximalOperator;
else
	proximalOperatorA = param.proximalOperatorA;
end
if ( ~isfield(param, 'proximalOperatorZ') )
    proximalOperatorZ = proximalOperator;
else
	proximalOperatorZ = param.proximalOperatorZ;
end
if ( ~isfield(param, 'proximalOperatorB') )
    proximalOperatorB = proximalOperator;
else
	proximalOperatorB = param.proximalOperatorZ;
end
...
if param.kA(1)>0
	...
	if isfield(param,'idx_indexA')
        weightsA = cell2mat(param.weights(param.idx_indexA));
    else
        weightsA = cell2mat(param.weights);
    end
else
...
if param.kZ(2)>0
	...
	if isfield(param,'idx_indexZ')
        weightsZ = cell2mat(param.weights(param.idx_indexZ));
    else
        weightsZ = cell2mat(param.weights);
    end
else
...
if param.kB(2)>0
	...
	if isfield(param,'idx_indexB')
        weightsB = cell2mat(param.weights(param.idx_indexB));
    else
        weightsB = cell2mat(param.weights);
    end
else
...
if use_Da
    if isfield(param,'Da_norm')
        ...
        if ( ~issparse(Da) && size(Da,1) < 100)
            Da_norm = norm(Da,2);
        else % for larger instances we use an approximation
            Da_norm = normest(Da);
        end
    end
else
...
[A,A_norm] = proximalOperatorA(A,0,param.kA,indexA,param.posA, [], weightsA);
[Z,Z_norm] = proximalOperatorZ(Z,0,param.kZ,indexZ,param.posZ, [], weightsZ);

if use_Db
    [B,B_norm] = proximalOperatorB(B,0,param.kZ,indexB,param.posB, [], weightsB);
else
    B_norm = 0;
end
...
while iter_count <= num_iter
    ...
    if param.start_A || do_A
        ...
        if ( ~issparse(varbl.ZTZ) && size(varbl.ZTZ,1) < 100)
            spectralNorm = norm(varbl.ZTZ,2);
        else % for larger instances we use an approximation
            spectralNorm = normest(varbl.ZTZ);
        end
        LA = spectralNorm*Da_norm^2;
        ...
        if param.save_dual
            [A(:,idx_nz),A_norm(idx_nz),dualA(:,idx_nz)] = ...
                proximalOperatorA(A_proj(:,idx_nz),param.lam*Z_norm(idx_nz)/LA,...
                param.kA,indexA,param.posA,dualA(:,idx_nz), weightsA, param.verbose);
        else
            [A(:,idx_nz),A_norm(idx_nz)] = ...
                proximalOperatorA(A_proj(:,idx_nz), ...
                param.lam*Z_norm(idx_nz)/LA,param.kA,indexA,param.posA, [], weightsA, param.verbose);
        end
    ...
    if iter_count <= num_iter
        ...
        if use_Da
            ...
            if ( ~issparse(varbl.ADDA) && size(varbl.ADDA,1) < 100)
                LZ = norm(varbl.ADDA,2);
            else % for larger instances we use an approximation
                LZ = normest(varbl.ADDA);
            end
        else
            % LZ = norm(varbl.ATA,2);
            if ( ~issparse(varbl.ATA) && size(varbl.ATA,1) < 100)
                LZ = norm(varbl.ATA,2);
            else % for larger instances we use an approximation
                LZ = normest(varbl.ATA);
            end
        end
		...
		if param.save_dual
			[Z(:,idx_nz),Z_norm(idx_nz),dualZ(:,idx_nz)] = ...
				proximalOperatorZ(Z_proj(:,idx_nz),param.lam*A_norm(idx_nz)/LZ,...
				param.kZ,indexZ,param.posZ,dualZ(:,idx_nz), weightsZ, param.verbose);
		else
			[Z(:,idx_nz),Z_norm(idx_nz)] = ...
				proximalOperatorZ(Z_proj(:,idx_nz), ...
				param.lam*A_norm(idx_nz)/LZ,param.kZ,indexZ,param.posZ, [], weightsZ, param.verbose);
		end
        ...
        if use_Db
            ...
            if param.save_dual
            [B,B_norm,dualB] = proximalOperatorB(B_proj,param.lam_B/LB, ...
                param.kB,indexB,param.posB,dualB, weightsB, param.verbose);
            else
             [B,B_norm] = proximalOperatorB(B_proj,param.lam_B/LB, ...
                param.kB,indexB,param.posB, [], weightsB, param.verbose);
            end
	...
	if param.verbose
		itTicTime = toc(itTic);
		disp(['iteration ' num2str(iter_count-1) ' took ' num2str(itTicTime) 's']);
	end
end
